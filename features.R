...
cor_mat_weight = round(cov.wt(data_set_origin[, -1], wt = data_set_origin$total_first_booking,
                        cor = TRUE)$cor, 2)
#fit data
fit <- lm(t3m_ttv_per_first_ttv ~ booking_per_customer + free_booking_percentage + 
              new_free_booking_percentage, data = data_set_origin)
summary(model)

accuracy = data.frame(total_first_booking=data_set_origin$total_first_booking
                      ,t3m_ttv_per_first_ttv=data_set_origin$t3m_ttv_per_1st_ttv,lmfitted=fit$fitted.values) 

ggplot(nonbrandLTV_accuracy,aes(x=lmfitted,y=t3m_TTV_per_1st_ttv))+geom_point(aes(size = total1st_bkg))
